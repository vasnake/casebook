...
Итак, в качестве результата поиска мы получаем (вынесем разные ошибки за скобки) две коллекции:
 - список дел (cases)
 - список сторон (sides)
...
ОАО «Глобинторг» (ИНН 7723830905)
    это очень интересный вариант - поиск по делам ничего не находит, но находит поиск по сторонам (sides) если указать ИНН http://casebook.ru/api/Search/Sides?name=7723830905&inn=7723830905
    запрос/ответ (3) в файле
